...
hash_t *rehash(hash_t *hash)
{
	...
	for (size_t w = 0; w < hash->capacidad; w++) {
		...
		for (size_t j = 0; j < tamanio; j++) {
			keyvalue_t *par =
				(keyvalue_t *)lista_elemento_en_posicion(
					listaAnterior, j);

			size_t nuevoIndice =
				funcion_hash(par->key, nuevaCapacidad);
		      ...
		      void **anterior)
{
	...
	if (!hash_contiene(hash, clave)) {
		...
		while (comparador(toInsert, lista_iterador_elemento_actual(
						    iterador)) != 0) {
			posicion++;
...
void *hash_quitar(hash_t *hash, const char *clave)
{
	...
	size_t posicion = lista_buscar_posicion(listaDondeBuscar, comparador,
						claveABuscar);
	keyvalue_t *elementoEncontrado = (keyvalue_t *)lista_quitar_de_posicion(
		listaDondeBuscar, posicion);
...
void *hash_obtener(hash_t *hash, const char *clave)
{
	...
	keyvalue_t *elementoAdevolver = lista_buscar_elemento(
		listaDondeBuscar, comparador, claveABuscar);
...
bool hash_contiene(hash_t *hash, const char *clave)
{
	...
	if (!lista_buscar_elemento(listaDondeBuscar, comparador,
				   claveABuscar)) {
		free((void *)claveABuscar->key);
		free(claveABuscar);
		return false;
