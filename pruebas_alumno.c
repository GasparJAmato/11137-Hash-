...
void prueba_crear()
{
	...
	pa2m_afirmar(hash_cantidad(hash) == 4,
		     "La cantidad de elementos deberia ser 4");
	pa2m_afirmar(dos == *(int *)hash_obtener(hash, "CLAVE 2"),
		     "La clave 2 tiene el valor 2");
	pa2m_afirmar(uno == *(int *)hash_obtener(hash, clave),
		     "La clave 1 tiene el valor 1");

	void *value1 = hash_quitar(hash, "CLAVE 1");
	pa2m_afirmar(*(int *)value1 == uno, "Se elimina CLAVE 1");
	pa2m_afirmar(!hash_obtener(hash, clave),
		     "CLAVE 1 no se encuentra en el hash");

	pa2m_afirmar(!hash_quitar(hash, "CLAVE 1"),
		     "No se puede eliminar una clave ya eliminada");

	hash_quitar(hash, "CLAVE 3");
	pa2m_afirmar(hash_cantidad(hash) == 2,
		     "Se elimina un elemento ahora la cantidad es 2");

	hash_quitar(hash, "CLAVE 128930");
	hash_insertar(hash, "CLAVE 2", &uno, &puntero_anterior);
	pa2m_afirmar(uno == *(int *)hash_obtener(hash, "CLAVE 2"),
		     "Se actualizo al valor 1");
	...
	hash_insertar(hash, "CLAVE 2", &dos, &puntero_anterior);
	pa2m_afirmar(dos == *(int *)hash_obtener(hash, "CLAVE 2"),
		     "Se volvio a actualizar el valor a 2");
